...
- 增加 `mdi_Service()` / `mdi_Clock()` 板级回调边界，由 `modus_Run()` 和
  `modus_Clock()` 统一调用。
- 将 ADC/DMA 块处理、均值发布、下一批采样调度和 Stream 超时维护收敛到板级 MDI 服务；
  DMA/USART 中断只发布所有权或搬运字节。
- 补充 Timer、Raw Tick、静态资源绑定以及 master/client 硬件 API 文档。

## [0.6.1.1] - 2026-09-19

### MDI 契约修复

- DMA 均值更新捕获本次发布序号后再确认，处理期间新增发布会保留到下一次更新；
  当生产速度超过环形缓冲容量时，处理最新有效块、重新同步消费序号并返回
  `MDI_OVERRUN`，避免消费端永久卡住。序号确认同时拒绝尚未发布的未来序号，并覆盖
  32 位序号回绕。
- GPIO 单向绑定宏现在在编译期约束能力方向：输入绑定必须只有 `MDI_IO_CAP_INPUT`，
  输出绑定必须只有 `MDI_IO_CAP_OUTPUT`，双向绑定必须同时声明两者。
- 硬件 I2C 将 `wTimeoutUs` 明确为每个阻塞总线阶段的最大时长。G431 默认按 170 MHz
  核心时钟和 32 个核心周期的轮询体校准为 5 polls/µs；修改时钟、Flash 等待状态或
  编译优化时必须由板级覆盖校准宏。
- 新增 `MDI_ENOTSUP` 和 `MDI_OVERRUN` 状态说明，并补充 DMA 竞争、溢出、回绕、GPIO
  方向负例和 I2C 错误优先级回归测试。
...
- 将公共 MDI 契约整理为 `src/mdi/core/`，保持芯片无关、编译期绑定和直接 provider
  调用；旧对象、`void *` 上下文和函数指针接口移入 `src/mdi/legacy/`。
- 将 ADC/DMA、软件 I2C、SPI EEPROM、FOC 等组合能力整理到 `src/mdi/feature/`，避免
  把具体应用流程和滤波策略扩张进 core。
- 增加 MDI 设计规范、应用接入说明、core/feature 职责说明和编译期调用开销验证。

### ADC/DMA 与参考实例

- 增加 ADC 单值读取、DMA 完成发布、稳定通道视图和批量均值 feature。
- 增加 `peripheral_template/` 多外设参考实例，覆盖 GPIO、并口 DAC、ADC/DMA、PWM、
  硬件/软件 I2C 和 SPI EEPROM。
- 明确 DMA 中断只发布完成状态，均值或其他滤波在任务/控制上下文执行；采样次数、
  通道布局和滤波算法通过实例或 feature 编译期扩展。
