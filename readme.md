...
> **当前工程约定：** M448 工程使用 `modus/src/mdi/core` 的原语和
> `modus/src/mdi/feature` 的可组合能力，由板级 `peripheral/<chip>/mdi/` 通过静态绑定
> 提供资源；不要按旧文档创建运行时 `HW` 或 `port_mdi.c` 包装层。
...
- **上位机支持**：配套 MStudio 实时绘制，支持 stream 和每变量刷新率。
